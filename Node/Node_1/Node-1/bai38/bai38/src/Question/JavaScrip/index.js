...
function QuestionJS() {
    ...
    const [showCorrectAnswers, setShowCorrectAnswers] = useState(false); // State for showing correct answers
    ...
    const toggleShowCorrectAnswers = () => {
        setShowCorrectAnswers(prevState => !prevState); // Toggle correct answer visibility
    };
                ...
                {data.length > 0 && data.map(item => (
                    <div className="question-icon" key={item.id}>
                        <ul className="dsquestion">
                            <li>
                                {item.questionsJavaScript?.map((value) => (
                                    <ul className="box" key={value.id}>
                                        ...
                                        {showCorrectAnswers && ( // Conditionally show correct answer
